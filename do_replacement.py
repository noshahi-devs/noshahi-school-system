...
# Find start of "Two Featured Items Row" section
...
# Find end of "</style>" section after featured cards
featured_section_start = start_idx
temp_idx = start_idx
style_count = 0
# Find all </style> after the start
parts = content[start_idx:].split('</style>')
if len(parts) < 2:
    print("ERROR: Could not find closing style tag")
    exit(1)

# Reconstruct to find the exact position
end_idx = start_idx + len(parts[0]) + len('</style>')
...
new_section = '''<!-- Two Featured Items Row - Premium Redesign -->
