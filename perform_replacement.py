...
# Find the start position (line 696: "<!-- Two Featured Items Row -->")
start_marker = "                <!-- Two Featured Items Row -->"

# Find the end position (line 822: end of the style tag)
# We need to find the closing </style> tag after the "FEATURED CARDS – RESPONSIVE STYLES" section
# Then check that the next line starts with "<!-- SECTION 3"
...
# Find end - look for the closing </style> that matches our section
if start_idx != -1:
    for i in range(start_idx, len(lines)):
        if '</style>' in lines[i] and i > start_idx + 50:  # Ensure we're looking after the start
            # Check if next line starts with the expected comment
            if i + 1 < len(lines):
...
print(f"Start index (line {start_idx + 1}): {lines[start_idx][:60]}")
print(f"End index (line {end_idx + 1}): {lines[end_idx][:60]}")
print(f"Next line: {lines[end_idx + 1][:60]}")
